...
// DDL identik dengan db/schema.sql — aman dijalankan berulang (idempotent).
const DDL = `
...
create table if not exists activity_logs (
  ...
  subbab integer not null, template_id text not null,
  is_correct boolean not null, error_detail text,
...
create table if not exists tutor_sessions (
  ...
  subbab integer not null, status text not null default 'REQUESTED',
  created_at timestamptz not null default now(),
...
create table if not exists summative_assessments (
  id serial primary key,
  student_id integer not null references students(id) on delete cascade,
  soal_1_answer jsonb not null, soal_2_answer jsonb not null,
  score_details jsonb not null, total_score integer not null,
...
async function schemaReady(): Promise<boolean> {
  const r = await db.execute(sql`
    select to_regclass('public.teachers') as t,
           to_regclass('public.students') as s,
           to_regclass('public.summative_assessments') as sa
  `);
  const row = r.rows[0] as { t: string | null; s: string | null; sa: string | null };
  return !!(row.t && row.s && row.sa);
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hint: ready
        ? undefined
        : "Tabel belum ada. Jalankan: POST /api/setup dengan body {\"seed\": true} (lihat README.md).",
    });
  } catch (e) {
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  let body: { key?: string; seed?: boolean } = {};
  try {
    body = await req.json();
  } catch {
    /* body kosong juga boleh */
  }

  // Jika SETUP_SECRET di-set di environment, wajib cocok.
  const secret = process.env.SETUP_SECRET;
  if (secret && body.key !== secret) {
    return NextResponse.json(
      { error: "Kunci salah. Kirim {\"key\": <SETUP_SECRET>} sesuai environment variable Anda." },
      { status: 401 }
    );
  }

  try {
    const wasReady = await schemaReady();
    if (!wasReady) {
      await db.execute(sql.raw(DDL));
    }
    const ready = await schemaReady();
    let seed: Awaited<ReturnType<typeof runDemoSeed>> | null = null;
    if (ready && body.seed) {
      seed = await runDemoSeed();
    }
    return NextResponse.json({
      ok: true,
      createdTables: !wasReady,
      schemaReady: ready,
      seeded: seed?.seeded ?? false,
      demo: seed?.seeded
        ? {
            guru: `${seed.teacherName} / ${seed.teacherPassword}`,
            kodeKelas: seed.classCode,
            passwordSiswa: seed.studentPassword,
            jumlahSiswa: seed.studentCount,
          }
        : undefined,
    });
  } catch (e) {
